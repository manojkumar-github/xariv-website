    ...
    <div className="relative">
      {v1Experience.map((step, i) => (
        <div key={step.step} className="relative flex gap-5 pb-8 last:pb-0">
          ...
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-accent bg-accent text-sm font-medium text-canvas">
            {step.step}
          </div>
          <div className="min-w-0 flex-1 pt-1">
            ...
            <Link href={step.href} className="mt-2 inline-block text-sm font-medium text-accent hover:underline">
              Start →
            </Link>
          </div>
        </div>
